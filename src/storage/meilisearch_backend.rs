use super::backend::{
    BatchCreateResult, BatchDeleteResult, DerivedIndexHealth, DerivedIndexStatus, HealthStatus,
    StorageBackend,
};
    ...
    fn health_check(&self) -> Result<HealthStatus, EngramError> {
        self.rt.block_on(async {
            match self.client.health().await {
                Ok(_) => {
                    let index_health = self.client.index(MEMORIES_INDEX).get_stats().await;
