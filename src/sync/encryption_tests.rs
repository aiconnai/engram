...
fn rotating_provider(active: u8, previous: u8) -> ConfiguredCloudKeyProvider {
    ConfiguredCloudKeyProvider::from_material_with_previous(
        EncodedKeyMaterial::Inline(Zeroizing::new(format!(
            "hex:{}",
            hex::encode([active; KEY_BYTES_LEN])
        ))),
        EncodedKeyMaterial::Inline(Zeroizing::new(format!(
            "hex:{}",
            hex::encode([previous; KEY_BYTES_LEN])
        ))),
    )
    .expect("rotating test provider loads")
}
...
#[test]
fn rotated_provider_decrypts_legacy_payload_with_previous_key() {
    // Given: legacy ciphertext written before rotation with the now-previous key.
    let old_writer = provider_from_byte(11);
    let rotated_reader = rotating_provider(12, 11);
    let plaintext = b"legacy rotation bytes";
    let legacy_payload =
        encrypt_legacy_payload_with_nonce(&old_writer, plaintext, [4u8; ENCRYPTION_NONCE_LEN])
            .expect("legacy payload encrypts");

    // When: the rotated provider reads the legacy payload.
    let decrypted = decrypt_data_with_provider(&rotated_reader, &legacy_payload)
        .expect("previous key decrypts legacy payload");

    // Then: read-old compatibility is preserved during the rotation window.
    assert_eq!(decrypted, plaintext);
}
...
fn malformed_versioned_payload_returns_redacted_error() {
    ...
    // When: decrypt is attempted.
    let error =
        decrypt_data_with_provider(&provider, &malformed).expect_err("malformed payload must fail");
