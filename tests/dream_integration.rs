...
fn test_dream_phase_integration() {
    ...
    storage.with_connection(|conn| {
        for i in 0..3 {
            conn.execute(
                ...
                params![format!("Rust programming language is safe and fast. Session {}.", i)],
            )?;
        }
        Ok(())
    }).unwrap();
    ...
    storage.with_connection(|conn| {
        let count: i64 = conn.query_row("SELECT COUNT(*) FROM dream_runs", [], |row| row.get(0))?;
        assert_eq!(count, 1);
        Ok(())
    }).unwrap();
    ...
    storage.with_transaction(|conn| {
        engram::storage::queries::acquire_dream_lock(conn, "dream_phase_all", "manual_owner", 3600)
    }).unwrap();
