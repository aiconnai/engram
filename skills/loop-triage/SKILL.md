---
name: loop-triage
description: Use for an Engram daily triage loop that safely collects and classifies CI, issue, PR, commit, and chat signals into report-only handoff state with bounded scope, no auto-fix, and loop-run-log updates.
---
...
Append a concise update to the loop state file, normally under
`docs/loops/<loop-name>/STATE.md` for repository-wide Engram loops:
