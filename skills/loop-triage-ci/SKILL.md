---
name: loop-triage-ci
description: Use for an Engram CI sweeper triage loop that groups CI failures, classifies root causes, caps attempts, and proposes bounded minimal-fix candidates only after budget, denylist, and verifier gates.
---
...
- Do not auto-fix dependency-major upgrades, auth, storage migrations,
  attestation/snapshot code, release publishing, production infrastructure, or
  other denylisted paths.
...
- The full Engram gate when agent, script, CI, connector, dependency, or policy
  surfaces changed: `bash docs/harness/bin/sensors.sh`.
